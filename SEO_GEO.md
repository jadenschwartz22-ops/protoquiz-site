...
- **llms.txt**: kept as the AI-facing summary (dated facts, article index) but know its limits — ~97% of llms.txt files got zero crawler requests in 2026 studies; no AI search system consumes it. It serves user-triggered agent fetches. Served with `X-Robots-Tag: noindex` (vercel.json). Update the "as of <month year>" facts when stats move; add flagship posts to "Notable articles".
...
- **About/author page**: real credentials (cert level, state, years) + Person schema + blog bylines linking to it. E-E-A-T alignment; content only Jaden can supply.
