...
- **llms.txt**: kept as the AI-facing summary (dated facts, article index) but know its limits — ~97% of llms.txt files got zero crawler requests in 2026 studies; no AI search system consumes it. It serves user-triggered agent fetches. Prod is GitHub Pages behind Cloudflare (push to main = deploy), which serves committed files with no header layer, so llms.txt is simply indexable. (A vercel.json once set `X-Robots-Tag: noindex` for it; that file was inert in prod and has been deleted.) Update the "as of <month year>" facts when stats move; add flagship posts to "Notable articles".
